...
class BRATSDataset3D_midslice(torch.utils.data.Dataset):
    def __init__(self, directory, transform, test_flag=False):
        '''
        directory is expected to contain some folder structure:
                  if some subfolder contains only files, all of these
    ...
    def __getitem__(self, x):
        ...
        for seqtype in self.seqtypes:
            nib_img = nibabel.load(filedict[seqtype])
            path=filedict[seqtype]
            o = torch.tensor(nib_img.get_fdata())[:,:,int(155/2)]
            # if seqtype != 'seg':
            #     o = o / o.max()
            out.append(o)
        out = torch.stack(out)
...
class BRATSDataset(torch.utils.data.Dataset):
    def __init__(self, directory, transform, test_flag=False):
        ...
        for f in files:
            seqtype = f.split('_')[3].split('.')[0]
            datapoint[seqtype] = os.path.join(self.directory, f)
    ...
    def __getitem__(self, x):
        ...
        if self.test_flag:
            ...
            if self.transform:
                image = self.transform(image)
            return (image, image, path.split('_')[-2]) # virtual path
        else:
            image = out[:-1, ...]
            label = out[-1, ...][None, ...]
            label=torch.where(label > 0, 1, 0).float()  #merge all tumor classes into one
            if self.transform:
                ...
                torch.set_rng_state(state)
                label = self.transform(label)
            return (image, label, path.split('_')[-2]) # virtual path